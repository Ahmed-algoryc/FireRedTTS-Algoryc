...
class FireRedTTS2:
    ...
    def generate_single(
        ...
        self._model.reset_caches()
        # Increase generation length to avoid truncating long sentences
        max_generation_len = 600
        tokens, tokens_mask = [], []
        for segment in context:
            segment_tokens, segment_tokens_mask = self._tokenize_segment(segment)
            tokens.append(segment_tokens)
        ...
        for _ in range(max_generation_len):
            ...
            # Hard stop to avoid cache overflow
            # Let the loop naturally stop based on max_generation_len
